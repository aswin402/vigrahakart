  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 min-h-[calc(100dvh-4rem)] md:min-h-[calc(100dvh-72px)]">
        {/* Left — Image */}
        <div className="relative h-[50vh] sm:h-[55vh] lg:h-auto overflow-hidden bg-warm-beige">
          <img
            src="/images/hero-statue.webp"
            alt="Handcrafted bronze statue in warm sunlight"
            className="w-full h-full object-cover object-center"
          />

          {/* Floating Product Card */}
          <div
            className={`absolute bottom-6 right-6 sm:bottom-10 sm:right-10 bg-cream/95 backdrop-blur-sm border border-border rounded-lg p-4 shadow-[0_8px_32px_oklch(0.28_0.06_45/0.12)] max-w-[200px] transition-all duration-700 delay-500 ${
              isVisible
                ? 'opacity-100 translate-y-0'
                : 'opacity-0 translate-y-4'
            }`}
          >
            <p className="text-[10px] font-body font-semibold uppercase tracking-[0.12em] text-temple-red mb-1">
              Handcrafted
            </p>
            <p className="text-sm font-heading font-semibold text-deep-brown mb-0.5">
              Bronze Ganesha Statue
            </p>
            <p className="text-xs text-muted-brown">From ₹2,499</p>
          </div>

          {/* Artisan Badge */}
          <div
            className={`absolute top-6 left-6 sm:top-10 sm:left-10 w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-temple-red flex items-center justify-center transition-all duration-700 delay-700 ${
              isVisible
        ...
        {/* Right — Content */}
        <div className="flex flex-col justify-center px-6 py-12 sm:px-10 md:px-14 lg:px-16 xl:px-20 bg-warm-beige/40">
          <div
          ...
          >
            ...
            <span className="inline-block text-xs font-body font-medium uppercase tracking-[0.2em] text-temple-red mb-5">
              Handcrafted Sacred Decor
            </span>

            {/* Heading */}
            <h1 className="font-heading text-4xl sm:text-5xl md:text-[3.5rem] lg:text-[3.75rem] xl:text-[4rem] font-semibold text-deep-brown leading-[1.1] mb-6">
              Timeless Statues for Sacred & Elegant Spaces
            </h1>

            {/* Description */}
            <p className="text-base md:text-lg text-muted-brown leading-relaxed mb-8 max-w-md">
              Discover handcrafted statues shaped with tradition, natural
